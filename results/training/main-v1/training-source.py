...
def train(config_path, stage, output, sft_adapter=None, resume=False, smoke=False):
    import torch
    ...
    class Progress(TrainerCallback):
        def on_log(self, args, state, control, logs=None, **kwargs):
            update = {"status": "training", "stage": stage, "step": state.global_step,
                "total_steps": state.max_steps, "epoch": state.epoch, "elapsed_seconds": time.time() - started, **(logs or {})}
