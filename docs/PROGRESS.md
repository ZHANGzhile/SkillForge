...
- 覆盖核验已完成：原B3失败中9例为train/validation均未覆盖的保留组合条件；超额退款已有3个train任务与35个SFT动作目标，仍有1个新实例失败。详见[边界覆盖](BOUNDARY_COVERAGE.md)。这不是将test补进训练的授权或新独立测试结论。

- 新增同模型B0/B3全78任务配对消融与自动失败归因；不改变冻结核心、权重或旧成绩。5项定向测试与107项独立源码回归通过（1项跳过），12:53已启动正式78例服务执行；桌面/手机进度卡片已检查通过。预声明范围和解释限制见[对照方案](REUSE_ABLATION_PLAN.md)，实时进度见[运行记录](REUSE_ABLATION_LIVE.md)。
