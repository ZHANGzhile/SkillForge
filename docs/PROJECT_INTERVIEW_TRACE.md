...
## 29. 2026-09-25实施追踪：从失败诊断到恢复训练

本节更新第28节的后续实施状态：main-v3已经开始训练，完整validation尚未完成，不能宣称退款问题已解决。原main-v2研究结果和HTTP4/6失败证据均保留。

### 29.1 为什么选择新课程，而不是先让Runtime自动纠正模型

已确认的两个产品失败分别是“退款写入后没有回读验证”和“重复读取至16步耗尽”。如果把缺失动作硬编码到Runtime，用户可能得到一次成功演示，但那不能说明Student学会了完整动作序列，也会改变与旧模型比较的执行条件。

本轮保留自由Action、16步预算、相同工具和原verifier。只增加train监督，让模型看到冗余历史之后的正确下一步，以及mutation后经过干扰仍需读取验证的上下文。这是针对已观察问题的工程假设；有效性必须由尚未完成的真实模型评测检验。

### 29.2 数据如何生成，为什么不会把循环本身奖励给模型

1. 输入固定为原69个train任务及原冻结Skill包。validation任务、自定义验收任务、新实例test均不作为optimizer样本。
2. 每个任务使用5种读取前缀，包括无前缀、一次订单读取、重复订单读取和多工具读取。每种前缀分别执行Skill优先、primitive优先、primitive写后插入读取干扰三种模式，共69×5×3=1,035条轨迹。
3. 所有轨迹实际经过Runtime和SQLite工具执行，按Expected Outcome Contract验收。教师是明确标记的确定性策略，llm_calls=0；这1,035条成功只证明教师数据通过审核，不是Student成绩。
4. 插入的前缀和写后干扰动作标记为injected_steps，不成为SFT目标。只提取它们之后教师实际选择的合法动作。退款写入后，目标应包含get_payment再stop；高风险任务仍应escalate。
5. 新增1,911个动作目标与原语料合并、去重后共1,267条SFT：tool541、stop361、refuse127、escalate121、skill117。原90对same-context DPO保留，本轮先训练SFT，不预设DPO必然更好。

具体脚本：`scripts/prepare_recovery_supervision.py`。审计清单：`results/training-data/recovery-v1/manifest.json`。完整上下文不截断，实际data_report中的oversize_skipped为0。切勿把“1,267个动作样本”说成“1,267个独立业务任务”。

### 29.3 如何避免从失败分析滑向测试集泄漏

旧test已经看过，后来调整过的模型再跑旧test只能叫回归。此次在训练前固定seed=20260925的新实例集，并与旧数据的task/order/instance ID检查不重叠。Skill仍绑定原编译数据hash；新评测入口另存training_dataset_hash与holdout_dataset_hash，不能把Skill来源重新标成新数据。

模型准入只看原完整validation：要求实际违规0、EOC至少达到旧部署DPO的63/69。达标后写入不可变选择记录，才让旧DPO和新SFT在同一78例新实例集评测。新test不参与模型选择。这个设计仍有两个限制：validation已被反复观察；新实例来自已知生成器与任务结构。因此不能把结果夸大成完全未知业务上的泛化能力。

### 29.4 为什么工程验收又加了一层状态链审核

一次新增测试发现：只把保存的初态、终态、tool_audit交给verifier重算，删掉tool_audit后仍可能得到相同成功结论。原因是写后验证规则根据审计中的mutation定位需要验证的位置；整个审计缺失会隐藏这段证据。

本轮没有改冻结verifier使旧分数发生变化，而是在发布边界增加完整性检查：从任务fixture生成初态；每条审计before_state必须等于上一条after_state；state_diff必须与两状态差异一致；最后一个after_state必须等于记录的最终状态。随后才重算EOC，并核对真实服务adapter指纹、llm_calls和free_action协议。测试删除退款审计或替换adapter都会失败。该检查证明证据内部一致，不是对模型来源的密码学证明。

### 29.5 面试中如何准确表述当前结果

- 已完成：原Base/SFT/DPO同协议对照、训练权重、逐任务证据、持久工作台、train恢复课程、新实例冻结、自动交付链路，以及102项源码独立回归与训练进度页面检查。
- 正在进行：main-v3的318次优化更新和完整validation；之后按准入结果进入新实例双评测及原六项HTTP验收。
- 尚未证明：恢复课程改善Student的最终成功率、两个退款验收已修复、复合任务已学会、最终交付全部通过。
- 归因限制：SFT样本从355增至1,267，在同为两轮训练时更新次数也增加；若后续更好，不能只归因于恢复数据内容，还混有计算量差异。

对应实施说明见[恢复训练方案](RECOVERY_V1.md)，实时阶段见[训练进度](TRAINING_LIVE.md)。训练loss下降、smoke权重有变化、浏览器页面正常，分别是三个局部检查，都不能代替实际业务验收。


## 30. 当晚真实结果：哪些改善，哪些仍失败

第29节的训练已完成。main-v3在完整validation中达到69/69，token loss0.013850；这允许它按预声明门槛进入新实例评测。其后才评测新实例test，未用test选择权重。
...
逐任务13例改善、2例退步，净增加11个成功，即14.1个百分点。这里的参考分数57/78来自新实例集；不能拿它替换main-v2原test的55/78。新test仍是已知生成器与结构中的新实例，训练仍是单seed且计算量变化，没有声明因果归因或统计显著性。

**为什么系统成功增加，固定候选决策反而少对一道？** 两项测试测量不同东西。固定候选探针只问给定上下文选Skill/refuse/escalate是否正确；完整系统还涉及读取、验证、停止及多步历史。恢复课程改善了多步执行，并不保证每个边界分类都改善。地址任务也有退步，应一起报告。

**两个真实产品失败修好了吗？** 同一验收脚本的普通退款和自定义部分退款都实际通过了，六项HTTP验收为6/6，真实浏览器桌面/手机及刷新恢复通过。普通退款调用了一次Skill，自定义部分退款通过primitive完成，共四次模型决策；没有把脚本教师接到真实服务。证据目录为results/workbench-acceptance/recovery-sft-20260925-154703。

**为什么还不能说所有退款和复合任务都解决了？** 新实例仍有10例失败：6例复合任务、3例地址边界错误拒绝，以及1例退款步数耗尽。功能验收覆盖六个固定案例，并不是任意输入成功保证。每个失败ID与原因见恢复结果报告。

**最后为什么在测试里又停了一次？** 取消任务是并发过程，cancel提交标记后，worker可能在调用返回读取状态前完成取消。旧测试硬性要求返回running会偶发失败；修复为事件同步，在worker提交终态前检查取消标记，再释放worker验证终态与审计。业务代码没改，既有冻结评测来源hash不变。

最终签收和ZIP回执见[交付状态](DELIVERY_STATUS.md)。可用于面试的表述是：完成了一个可追溯的本地Skill学习研究闭环，恢复课程在同一新实例集上改善了系统EOC并通过真实服务功能验收，同时保留失败、退步与实验边界。


## 31. 继续追问：69/69验证是否证明边界问题已经解决

不能。9月26日新增的只读覆盖审计先复核main-v3的1,267个去重SFT样本及完整test检查点，再数条件覆盖：

| 条件 | train任务 | validation任务 | SFT动作目标 | 新test合格 |
|---|---:|---:|---:|---:|
| 高风险且已发货 | 0 | 0 | 0 | 0/3 |
| 无效地址后继续尝试取消 | 0 | 0 | 0 | 0/3 |
| 已发货后按复合请求转人工 | 0 | 0 | 0 | 0/3 |
| 退款金额超出剩余额度 | 3 | 3 | 35 | 2/3 |

前三类是原数据设计刻意保留的组合条件，失败体现组合泛化不足，不能因为没有train样本就判定数据管线错误。后一类已具备监督，仍有一次反复读取，不能用“没有见过”解释全部失败。扩充下一版训练前需要明确新的研究范围，重新设计验证及未知测试结构；不能将这些已查看test任务改标签后继续称独立测试。

**68/78能证明Skill带来多少收益吗？** 暂时不能。旧模型与新模型同时改变了训练数据和训练计算量。为此已实现同一main-v3权重、同一78例任务的B0/B3配对消融：B0完全移除Skill与Memory，B3引用已审核检查点。服务模型settings与已发布模型逐项一致，网络/GPU故障不混入能力分母，全部任务执行后才输出对照成绩。

**为什么还不把配对退步率叫causal NTR？** B0取消Skill也取消自动Gate补读，可见状态和后续上下文会改变。它回答“这个完整系统配置是否更好”，不直接回答“在完全相同决策上下文中，这次Skill执行是否导致失败”。输出会区分所有任务上的退步率、B0成功条件下的退步率、退步且实际尝试Skill的任务数；同上下文动作级causal NTR仍为null。

本节新增对照尚在执行，不提前写出模型成绩。范围见[消融方案](REUSE_ABLATION_PLAN.md)，进度见[实时记录](REUSE_ABLATION_LIVE.md)，覆盖原始证据见[边界报告](BOUNDARY_COVERAGE.md)。
